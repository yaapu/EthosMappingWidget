...
function drawLib.drawTopBarSensor(widget,x,sensor,label)
  ...
  if sensor ~= nil then
    ...
    drawLib.drawText(x-w-2, 10, label == nil and sensor:name() or label, FONT_XS, status.colors.barText, RIGHT)
...
function drawLib.drawTopBar(widget)
  ...
  drawLib.drawText(0, 0, status.modelString ~= nil and status.modelString or model.name(), FONT_L, status.colors.barText, LEFT)
  ...
  if status.conf.userSensor1 ~= nil and status.conf.userSensor1:name()  ~= "---" then
    ...
    offset = offset + drawLib.drawTopBarSensor(widget, 800-offset, status.conf.userSensor1) + 6
  end
  if status.conf.userSensor2 ~= nil  and status.conf.userSensor2:name() ~= "---" then
    offset = offset + drawLib.drawTopBarSensor(widget, 800-offset, status.conf.userSensor2) + 6
  end
  if status.conf.userSensor3 ~= nil and status.conf.userSensor3:name() ~= "---" then
    offset = offset + drawLib.drawTopBarSensor(widget, 800-offset, status.conf.userSensor3) + 6
  end
end
